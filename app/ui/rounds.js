...
var RoundsUI = (function () {
  ...
  // ⚠️ 业务逻辑全在 core/roundflow.js。这里只留几个同名短别名,
  //    是为了让原来那几十个调用点不用逐个改 —— 逐个改的风险大于收益,
  //    而且漏一个就是「这个按钮点了没反应」那种最难查的。
  //
  // ⚠️ **别在这个文件里写 Store.set。** 存储写入一律走 RoundFlow ——
  //    界面还要为小程序重写一遍,业务留在渲染层就得写两份,
  //    两份状态机必然漂移。tools/check.sh 有一条 grep 守着这件事。
  var variantOf = RoundFlow.variantOf;
  var mealIngredients = RoundFlow.mealIngredients;
  var mealSeasonings = RoundFlow.mealSeasonings;
  var savePkgCorrection = RoundFlow.savePkgCorrection;
  var markAllBought = RoundFlow.markAllBought;
  function saveLog(r, log) { RoundFlow.saveLog(r, log); r.log = log; }
  ...
  function create() {
    RoundFlow.create({ days: draft.days, perDay: draft.perDay,
                       diners: draft.diners, overrides: draft.overrides });
    sheetOpen = false;
    render();
  }
  ...
  /** 排一轮。业务在 RoundFlow.generate,这里只负责「排不出来怎么说」。 */
  function generate(r, silent) {
    var out = RoundFlow.generate(r);
    if (out.ok) { render(); return true; }
    if (!silent) {
      Modal.note({
        title: '这次没排出来',
        body: '原因:' + out.reason + '。多半是约束太紧 —— ' +
              '把耗时上限放宽,或者少勾几样忌口再试。',
      });
    }
    return false;
   ...
   */
  /** 排不出来时说什么 —— resolveRound 和 applySwap 共用,别写两份 */
  function noteUnsolvable() {
    Modal.note({
      title: '这样就排不出来了',
  ...
  /** 改点什么再重新求解,排不出来就整个撤销(逻辑在 RoundFlow.resolve)。 */
  function resolveRound(id, mutate) {
    var out = RoundFlow.resolve(id, mutate);
    render();
  ...
  function toggleBought(r, ingredientId) {
    RoundFlow.toggleBought(r, ingredientId);
    render();
  }
  ...
  function setActual(r, ingredientId, grams) {
    RoundFlow.setActual(r, ingredientId, grams);
    render();
  }
  ...
  function applySwap(r, m, ban, dish) {
    var out = RoundFlow.applySwap(r, m, ban, dish);
    render();
    if (!out.ok) noteUnsolvable();
  }

  function toggleCooked(r, recipeId) {
    var out = RoundFlow.toggleCooked(r, recipeId);
    // ⚠️ **点完「做了」自动翻到下一顿。**
    ...
    if (out && out.cooked) { openMeal = null; jumpToOpen = true; }
    render();
  }

...
var Solver = (function () {
  ...
  var DEFAULT_WEIGHTS = {
    ...
    // 开局 7 味、买了的真进柜子;不限预算):
    //     4 → 54.3 味  44.9 道  25.0%
    //     8 → 55.0 味  45.4 道  25.1%
    //    12 → 49.3 味  42.2 道  25.5%
    //    20 → 44.2 味  42.6 道  26.1%   ← 取这个
    //    25 → 43.1 味  42.3 道  25.7%
    //    30 → 36.7 味  41.2 道  25.5%
    //    40 → 32.4 味  39.6 道  24.4%
    // (柜子 / 半年吃到的不同菜数 / 生鲜浪费率)
    //
    // ⚠️ **上面那张表测的运行条件不存在。** 它是在「不限预算」下扫的,
    //    而默认预算是 1 —— 两个东西线上同时生效。按实际条件再扫一遍
    //    (预算固定 1 味):
    //         4 → 19.9 味  41.7 道  23.1%
    //        12 → 19.2 味  41.2 道  23.1%
    //        20 → 18.9 味  41.3 道  22.8%   ← 取这个,浪费率最低
    //        30 → 18.3 味  41.4 道  23.6%
    //        40 → 18.3 味  41.1 道  23.8%
    //
    //    **硬预算一旦生效,这个权重就退居次要**:权重翻十倍柜子只差
    //    1.6 味(不限预算时差 22 味),菜品种类几乎不动。它剩下的作用
    //    是「闸门之内怎么选」,而 20 在那个尺度上是浪费率最低点。
    //    换句话说:柜子大小归预算管,权重只管选得好不好 —— 别让它去
    //    买预算已经买到的东西。
    //
    //    (顺带,20 也正好是「一瓶新调料 = 多买一样生鲜」的价 ——
    //     extraItems 也是 20。两条独立的理由指向同一个数。)
    //
    // ⚠️⚠️ 上一版这里写的是「20 是浪费率的最低点」,那批数字作废了:
    //    当时四个模拟脚本都漏了 global.Meal,solver 里配菜/蛋白门槛/
    //    份量缩放整块静默跳过,扫出来的是残缺求解器的行为。
    //    **注释里的数字必须标明是在什么环境下测的**,否则它会以
    //    「看起来很严谨」的样子把人带偏 —— 比没有数据更糟。
    //
    // ⚠️ **别指望这一项能解决问题。** 权重从 4 提到 40(十倍),
    //    半年后的柜子也才从 54 味降到 32 味 —— 正常人家里是 10~20 味。
    //    扣分只能让「买新调料」变得不划算,拦不住它。真正的闸是
    //    constraints.newSeasoningBudget(见 stage2):
    //        预算 0 → 13.3 味    1 → 18.9 味    2 → 27.7 味    不限 → 44.2 味
    //    而且收紧预算**同时降低浪费率**(22.8% vs 不限的 26.1%)——
    //    被逼着在现有调料能做的菜里深挖,食材复用反而更好。
    missing: 20,

...
var Nutrition = (function () {
   ...
   */
  // 判据:**能蒸/煮出来、往盘子边上一放就是一份主食的**。
  //    ✓ 谷物(米/杂粮)+ 淀粉类根茎瓜果(红薯/土豆/芋头/玉米/贝贝南瓜)
  //    ✗ 面条类 —— 那是**菜本身**(一碗面),不是配在菜旁边的那份
  //    ✗ 面粉 / 米粉 / 糯米粉 —— 是原料,不是一碗饭
  //    ✗ 豆类(芸豆/鹰嘴豆)—— 拌得进饭里,单独当主食没人这么吃
  //    ✗ 燕麦片 —— 早餐粥,配红烧肉很怪
  //
  // ⚠️ 第一版只有 6 样,把**玉米/土豆/芋头/贝贝南瓜全漏了** ——
  //    而字典里这几样 `countsAsVeg` 都是 false,早就按主食归类了,
  //    是我手点的时候没照着数据看。玉米 96 kcal、蒸一根当主食,
  //    减脂时和红薯一样是低能量密度的好选择。
  //
  // ⚠️ 加进来的必须 `countsAsVeg === false`,否则同一样东西
  //    既算主食又算蔬菜,蔬菜达标率会凭空虚高。
  //    南瓜(26 kcal)和莲藕就是 true,所以用的是贝贝南瓜(90 kcal)。
  var STAPLE_CHOICES = ['rice', 'brown_rice', 'foxtail_millet', 'job_tears', 'quinoa',

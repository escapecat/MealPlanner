...
if (loaded === srcs.length) {
  ...
  (function () {
    ...
    try {
      ...
      //    不报错,只是内容够不着。
      // ⚠️ 开关**展开时也得在**,不然点一下它自己就没了(「按钮点了就消失」)。
      //    而且两个开关**各管各的** —— 点菜单不该把采购清单收起来,
      //    否则整页往上跳一大截,滚动位置全乱。
      var bar = clickable(node, '这几天做什么');
      ok(!!bar, '待采购阶段有「这几天做什么」这一条');
      if (bar) {
        fire(bar);
        ok(txt(node).indexOf('一道菜 = 一顿') >= 0, '点开之后菜单真的展开了');
        ok(!!clickable(node, '这几天做什么'), '展开之后那一条还在(不能点一下就消失)');
        ok(txt(node).indexOf('还要买') >= 0, '点菜单不该把采购清单收起来 —— 两个开关各管各的');
        fire(clickable(node, '这几天做什么'));
        ok(txt(node).indexOf('一道菜 = 一顿') < 0, '再点一下能收回去');
      }
    } catch (e) {
